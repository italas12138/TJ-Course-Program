...
int m;
char chucun[1000000], chazhao, s[1000000];
...
void inorder(Bittree T1,char chazhao)
{
	...
	if (p->data == chazhao)
	{
		if (p->rightchild != NULL)
			...
			{
				Bittree fuzhu = p->rightchild;
				while (fuzhu->ltag != 1)
					fuzhu = fuzhu->leftchild;
				cout << "succ is " << fuzhu->data << fuzhu->rtag << endl;
			}
		else
		...
		if (p->leftchild != NULL)
			...
			{
				Bittree fuzhu = p->leftchild;
				while (fuzhu->rtag != 1)
					fuzhu = fuzhu->rightchild;
				cout << "prev is " << fuzhu->data << fuzhu->ltag << endl;
			}
		else
